                ...
                <ReactMarkdown
                  components={{
                    pre: (preProps) => {
                      ...
                      // Extract code content safety with type checking
                      let codeContent = '';
                      let language = '';
                      ...
                      return (
                        <div className="relative overflow-auto w-full my-2 bg-black/10 p-2 rounded-lg group">
                          <button 
                            onClick={() => {
                              navigator.clipboard.writeText(codeContent);
                              toast.success("Code copied to clipboard!");
                            }}
                            className="absolute right-2 top-2 p-1 rounded bg-gray-800 text-white opacity-0 group-hover:opacity-100 transition-opacity"
                    ...
                    code: ({node, inline, className, children, ...props}) => {
                      if (inline) {
                        return <code className="bg-black/10 rounded-lg p-1" {...props}>{children}</code>;
                      }
                      return null; // Code blocks are handled by pre
                    }
                  }}
...
//   const onSubmit = async (values: z.infer<typeof formSchema>) => {
//     try {
//       // const userMessage: createCompletion = { model: "user", content: values.prompt };
//       const userMessage: ChatCompletionRequestMessage = { role: 'user', content: values.prompt };
